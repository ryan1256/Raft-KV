...
//在C++中，raftRpc 类是通过公有继承（public）从 ::PROTOBUF_NAMESPACE_ID::Service 类继承而来的。
//这里的 ::PROTOBUF_NAMESPACE_ID::Service 是Google Protocol Buffers（简称Protobuf）库中的一个类，用于表示一个服务接口，该接口可以被RPC（远程过程调用）框架使用来在不同节点或进程之间通信。
